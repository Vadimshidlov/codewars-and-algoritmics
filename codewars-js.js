...
const res = Object.keys(classNamesCount).sort((a, b) => classNamesCount[b] - classNamesCount[a])
console.log(res)
*/
// Puzankov Task Valuta
/*
const input = [
  ["usd", "buy", 10000],
  ["usd", "sell", 5000],
...
const resultTwo = {}
input.forEach(item => {
  let [currency, type, count] = item
  if (!resultTwo[currency]) {
    resultTwo[currency] = [0, 0]
  }
  resultTwo[currency][type === "buy" ? 0 : 1] += count
})
...
// ПРАВИЛЬНЫЕ СКОБОЧКИ //подумать над реализацей немного позже
/*
let s1 = "()"
let s2 = "()[]{}"
let s3 = "(]"
